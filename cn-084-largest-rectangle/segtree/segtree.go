...
type node struct {
	startIndex int
	endIndex   int // [start, end] include end
...
func makeNode(start int, end int, values Interface, parent *node) *node {
	...
	n.right = makeNode(mid+1, end, values, n)
...
func New(values Interface) *SegmentTree {
	...
	st.root = makeNode(0, values.Len()-1, values, nil)
	return st
}
...
func searchNode(n *node, start int, end int, values Interface) int {
	...
	rightIndex := searchNode(n.right, mid+1, end, values)
