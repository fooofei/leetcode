...
// 我们现在的处境是 [0,j-1] 都是匹配的，j 发生不匹配,不能写转移到 j+1 状态
// 则按照朴素比较的处理逻辑应该是模式串向后移动一位，
// 跟上一句结合起来就是要重新匹配 [1,j-1] 之间的字符，
// 因为我们现在 DFA 是 j 状态前面的部分已经构建好了
// 因此可以直接拿来使用，就是 DFA 状态转化从 0 状态开始依次输入 [1,j-1] 个输入
// 把这个作为 X，然后 dfa[p[j]][x] 就是 j 不匹配时应该转到的下一个状态
// 到这里理解就通畅了
...
func buildDFA(pat []byte) [][]int {
	...
	// LPS 总是跟踪着除了 pat 首字符，剩下的字符跳转的状态
	for j := 1; j < len(pat); j++ {
		dumpMatrix(T, len(pat), uniqChars, "array")
		c := pat[j]
		for _, uc := range uniqChars {
			// 矩阵的含义为 在 j 状态输入 uc 跳转到哪个状态
